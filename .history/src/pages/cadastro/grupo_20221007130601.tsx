...
export default function CadastrarCidade({grupo,cidade, plano}){
    ...
    const [form, setForm] = useState<DataForm>({descricaoGrupo: '', planoPertencente: '',
     cidadePertencente: null, isDisable: true, cidadeSelecionada: undefined});
            ...
            .then(() => {
                setForm(
                    {descricaoGrupo : '', planoPertencente: '', cidadePertencente: null,
                      isDisable:true,cidadeSelecionada: undefined}
                )
            router.replace(router.asPath)
            })
        }catch(error){
                                        ...
                                        <div className="formGrande">
                                                    ...
                                                    <InputLabel id="demo-simple-select-autowidth-label">Cidade pertencente *</InputLabel>
                                                        <Select
                                                            ...
                                                            onChange={e=> setForm({...form, cidadePertencente : e.target.value, 
                                                             cidadeSelecionada: Number(form.cidadePertencente),
                                                             isDisable : false})}
                                                            autoWidth
                                            ...
                                            <div id='campo2'>
                                                <FormControl sx={{m : 0, minWidth: 220}}>
                                                    ...
                                                    {
                                                        plano.map(plano => {
                                                            if(plano.idkey_cidade == Number(form.cidadePertencente)){
                                                                console.log(`IDKEY CIDADE: ${plano.idkey_cidade}`);
                                                                <MenuItem value={plano.idkey}>{` PLANO DE GESTAO`}</MenuItem>
                                                            }
