...
export default function CadastrarCidade({link, grupo, plano, cidade}){
    ...
    isDisabled4 : true, isDisabled5 : true })
    ...
    async function submitForm(data : DataForm){
        try{
            ...
            .then(() => {
                ...
                isDisabled3 : true, isDisabled4 : true, isDisabled5 : true})
            router.replace(router.asPath)
            })
            ...
            <div>
                        ...
                        <div className="filtros">
                                ...
                                <FormControl sx={{m : 0, minWidth: 220}}>
                                    ...
                                    <Select
                                        ...
                                        id="demo-simple-select-autowidth"
                                        value={cidade.nome}
                                        onChange={e=> setForm({...form, planoSelecionado : e.target.value, isDisabled4 : false})}
                                        autoWidth
                                        label="Plano:"
                                        required
                                ...
                                <FormControl sx={{m : 0, minWidth: 220}}>
                                    ...
                                    >
                                        {
                                            grupo.map(function(grupo){
                                                if(form.planoSelecionado == link.grupo.plano.idkey){
                                                    return(
                                                        <MenuItem value={grupo.idkey}>{grupo.descricao}</MenuItem>
                                                    )
                                                }
                    ...
                    {    
                        link.map(function(link){
                            if(form.cidadeSelecionada == link.grupo.plano.cidade_idkey && form.planoSelecionado == link.grupo.idkey_plano
                            && form.grupoSelecionado == link.idkey_grupo)
                            {
                                return(
                                    <CardListagemDocumento id={link.idkey} descricao={`${link.descricao} ${link.grupo.plano.cidade.nome}`}/>
                                )
                            }
                        })
                    }
