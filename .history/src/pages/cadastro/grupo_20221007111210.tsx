...
export default function CadastrarCidade({grupo, plano, cidade}){
    console.log(cidade[0].plano[0].descricao)

    const router = useRouter();
    const [form, setForm] = useState<DataForm>({descricaoGrupo: '', planoPertencente: '',
     cidadePertencente:'', isDisable: true});
    ...
    async function submitForm(data : DataForm){
        ...
        try{
            ...
            .then(() => {
                setForm(
                    {descricaoGrupo : '', planoPertencente: '', cidadePertencente: '', isDisable:true}
                )
            router.replace(router.asPath)
            })
        }catch(error){
    ...
    async function clearForms(data : DataForm){
        setForm({descricaoGrupo : '', planoPertencente: '', cidadePertencente: '', isDisable:true})
    }
                                            ...
                                            <div id='campo2'>
                                                <FormControl sx={{m : 0, minWidth: 220}}>
                                                    ...
                                                    {                  
                                                        plano.map(plano => {
                                                            return(
                                                                <MenuItem value={cidade.plano.idkey}>{`${cidade.plano.descricao}`}</MenuItem>
                                                            )
                                                        })                                                                                        
                                                    }    
                                                    </Select>
